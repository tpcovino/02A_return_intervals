...
# Lab 2 - Return intervals, rational method, and SCS curve numbers

- In this portion of lab 2, Lab 2A, we will focus on return intervals. Lab 2B will focus on the rational method and curve numbers.

- Lab 2A (14 pts)

## Reading 

Rational method. Read at least sections 2 and 3.<br> 
- [Link](https://www.waternz.org.nz/Attachment?Action=Download&Attachment_id=846){target="_blank"}

NRCS module on estimating runoff using curve number (CN). <br>
- [Link](https://cdn.serc.carleton.edu/files/geoinformatics/steps/presentation_selecting_cn_vari.pdf){target="_blank"}

Slides on selecting curve number.<br>
- [Link](https://cdn.serc.carleton.edu/files/geoinformatics/steps/presentation_selecting_cn_vari.pdf){target="_blank"}

Time of concentration. Up to "other considerations", pages 15-1 to 15-9. <br>
- [Link](https://directives.sc.egov.usda.gov/OpenNonWebContent.aspx?content=27002.wba#:~:text=Time%20of%20concentration%20(Tc)%20is,the%20watershed%20to%20the%20outlet.){target="_blank"}
...
After computing some return intervals we will then use some of the simpler rainfall-runoff modeling approaches (the rational method and the curve number method) to simulate runoff for a hypothetical basin. 
...
# that's still a lot of stations. Let thin for "GALLATIN"

station_data_bzn <- station_data %>% 
  filter(str_detect(name, "BOZEMAN")) %>% 
  # get rid of duplicate ids to make plotting easier
  dplyr::distinct(id, .keep_all = TRUE)

# Now, let's create a zoomable map with the stations around Gallatin County. Click around and find the station id for the precip gauge with the LONGER timeseries. If you click on a station symbol, the first and last data year will appear. Obviously you can get this information from the station_data_gal data frame, but I wanted to show you the mapping capabilities. 
...
```{r}
climate_data <-   meteo_pull_monitors(c("USC00241044"),
...
There is a large snow event in 1951. We can assume that is "real", so let's keep it in the analysis. But you should think through how you could exclude it from the analysis. 

Next, we want to use some skills from Lab 1 to add a WY column. Try that here. 
```{r}
...
Now let's calculate some probabilities. Look up the `pnorm()` function for this (either type it into the Help window, or type ?pnorm in the console. You only need x, the mean, and sd for the calculations.

### **Q1. (2 pts) What is the probability that the annual precipitation in a given year is less than 400 mm? This is the F(A) in the CDF in the lecture slides.** 
